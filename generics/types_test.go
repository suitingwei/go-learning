...
type PersonContainer[S stringer, P plusser] struct {
	stringHandler S
	plusser       map[string]P           //这里可以类似原来的形式使用泛型类型
	next          *PersonContainer[S, P] //支持递归(自我引用),可以生成类似链表的结构
}

//当创建函数的时候，不需要再添加type parameter的具体类型，比如以下声明是错误的
//func (c PersonContainer[S stringer,P plusser])  FuncName() string{}
//而是直接使用type parameter即可
func (c PersonContainer[S, P]) GetFinalName() string {
