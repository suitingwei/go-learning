...
func LearnReflection() {
	...
	//7.Value of
	va, vp := reflect.ValueOf(intVar), reflect.ValueOf(&intVar)
	fmt.Println(va.CanAddr(), va.CanSet())
	fmt.Println(vp.CanAddr(), vp.CanSet())   //指针也是不能取地址，不能修改值的
	vpe := reflect.ValueOf(&intVar).Elem()   //指针的元素这么取出来就可以了
	fmt.Println(vpe.CanAddr(), vpe.CanSet()) //指针也是不能取地址，不能修改值的
}
